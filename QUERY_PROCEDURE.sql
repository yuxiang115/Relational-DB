...
WITH nn AS (SELECT cname FROM [dbo].[purchased] WHERE pid in (SELECT pid FROM [dbo].[product2] WHERE color = @color))
SELECT *
FROM [dbo].[customer] 
WHERE cname IN (SELECT cname FROM [dbo].[purchased] WHERE pid = (SELECT COUNT(pid) FROM [dbo].[product2] WHERE color = @color))
END
GO
